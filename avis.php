<?php
include('header.php');
 ?>


<!-- Start Qui sommes nous -->
<section class="bg-qui-sommes-nous">
  <div class="container-fluid">
    <div class="row ">
      <div class="col-lg-5 bg-lines-qui-sommes-nous-mobile">
        <h2 class="titre-qui-sommes-nous text-white pb-5 pt-5">Qui sommes-nous ?</h2>
        <p class="text-white paragraphe-size-qui-sommes-nous">Sport Design Automobiles est une jeune société spécialisée
        dans la recherche personnalisée de véhicule toute marque,
        neuf ou occasion, dotée de professionnels de l’automobile
        cûmulant plus de 10 ans d’expérience.
        </p>
        <p class="text-white paragraphe-size-qui-sommes-nous">Notre priorité ? La satisfaction du client. Nous mettons tout
        en œuvre pour vous trouver LE véhicule qui vous correspond
        en fonction de vos critères.
        </p>
        <p class="text-white paragraphe-size-qui-sommes-nous">Notre spécialité ? La recherche personnalisée de véhicule neuf
        ou occasion, toute gamme et toute marque confondue.
        </p>
        <p class="text-white paragraphe-size-qui-sommes-nous">N’hésitez pas à contacter nos commerciaux qui se feront un
        plaisir de vous renseigner.
        </p>
      </div>
      <div class="col-lg-7 hover-presentation bg-photo-presentation text-center">
          <div class="text-center overlay-presentation">
            <h3 class="d-inline-block text-uppercase text-white text-center">l'avis de nos clients</h2>
              <img src="img-avis/Qui_Sommes_Nous/car.png" alt="car" class="car-icon">
            <h4 class="text-uppercase text-white">qualité de notre accueil</h4>
              <ul class="d-flex flex-row justify-content-center">
                <li>
                  <span><img src="img-avis/Qui_Sommes_Nous/star-black.png" alt="star-black" class="mr-4"></span>
                </li>
                <li>
                  <span><img src="img-avis/Qui_Sommes_Nous/star-black.png" alt="star-black" class="mr-4"></span>
                </li>
                <li>
                  <span><img src="img-avis/Qui_Sommes_Nous/star-black.png" alt="star-black" class="mr-4"></span>
                </li>
                <li>
                  <span><img src="img-avis/Qui_Sommes_Nous/star-black.png" alt="star-black" class="mr-4"></span>
                </li>
                <li>
                  <span><img src="img-avis/Qui_Sommes_Nous/star-white.png" alt="star-white" ></span>
                </li>
              </ul>
            <h4 class="text-uppercase text-white text-center">nos infrastructures</h4>
              <ul class="d-flex flex-row justify-content-center">
                <li>
                  <span><img src="img-avis/Qui_Sommes_Nous/star-black.png" alt="star-black" class="mr-4"></span>
                </li>
                <li>
                  <span><img src="img-avis/Qui_Sommes_Nous/star-black.png" alt="star-black" class="mr-4"></span>
                </li>
                <li>
                  <span><img src="img-avis/Qui_Sommes_Nous/star-black.png" alt="star-black" class="mr-4"></span>
                </li>
                <li>
                  <span><img src="img-avis/Qui_Sommes_Nous/star-black.png" alt="star-black" class="mr-4"></span>
                </li>
                <li>
                  <span><img src="img-avis/Qui_Sommes_Nous/star-white.png" alt="star-white" ></span>
                </li>
              </ul>
            <h4 class="text-uppercase text-white">notre professionnalisme</h4>
              <ul class="d-flex flex-row justify-content-center">
                <li>
                  <span><img src="img-avis/Qui_Sommes_Nous/star-black.png" alt="star-black" class="mr-4"></span>
                </li>
                <li>
                  <span><img src="img-avis/Qui_Sommes_Nous/star-black.png" alt="star-black" class="mr-4"></span>
                </li>
                <li>
                  <span><img src="img-avis/Qui_Sommes_Nous/star-black.png" alt="star-black" class="mr-4"></span>
                </li>
                <li>
                  <span><img src="img-avis/Qui_Sommes_Nous/star-black.png" alt="star-black" class="mr-4"></span>
                </li>
                <li>
                  <span><img src="img-avis/Qui_Sommes_Nous/star-white.png" alt="star-white" ></span>
                </li>
              </ul>
            <h4 class="text-uppercase text-white text-center">suivi de commande</h4>
              <ul class="d-flex flex-row justify-content-center">
                <li>
                  <span><img src="img-avis/Qui_Sommes_Nous/star-black.png" alt="star-black" class="mr-4"></span>
                </li>
                <li>
                  <span><img src="img-avis/Qui_Sommes_Nous/star-black.png" alt="star-black" class="mr-4"></span>
                </li>
                <li>
                  <span><img src="img-avis/Qui_Sommes_Nous/star-black.png" alt="star-black" class="mr-4"></span>
                </li>
                <li>
                  <span><img src="img-avis/Qui_Sommes_Nous/star-black.png" alt="star-black" class="mr-4"></span>
                </li>
                <li>
                  <span><img src="img-avis/Qui_Sommes_Nous/star-white.png" alt="star-white" ></span>
                </li>
              </ul>
            <h4 class="text-uppercase text-white text-center">véhicule à la livraison</h4>
              <ul class="d-flex flex-row justify-content-center">
                <li>
                  <span><img src="img-avis/Qui_Sommes_Nous/star-black.png" alt="star-black" class="mr-4"></span>
                </li>
                <li>
                  <span><img src="img-avis/Qui_Sommes_Nous/star-black.png" alt="star-black" class="mr-4"></span>
                </li>
                <li>
                  <span><img src="img-avis/Qui_Sommes_Nous/star-black.png" alt="star-black" class="mr-4"></span>
                </li>
                <li>
                  <span><img src="img-avis/Qui_Sommes_Nous/star-black.png" alt="star-black" class="mr-4"></span>
                </li>
                <li>
                  <span><img src="img-avis/Qui_Sommes_Nous/star-white.png" alt="star-white" ></span>
                </li>
              </ul>
            </div>
          </div>
        </div>
  </div>
</section>
<!-- End Qui sommes nous -->

<!-- Start Avis -->
<section class="bg-image">
  <!--CAROUSEL-->
    <div id="carousel" class="carousel slide" data-ride="carousel">
      <ol class="carousel-indicators">
        <li data-target=".carousel" data-slide-to="0" class="active"> </li>
        <li data-target=".carousel" data-slide-to="1"></li>
        <li data-target=".carousel" data-slide-to="2"></li>
        <li data-target=".carousel" data-slide-to="3"></li>
        <li data-target=".carousel" data-slide-to="4"></li>
        <li data-target=".carousel" data-slide-to="5"></li>
        <li data-target=".carousel" data-slide-to="6"></li>
      </ol>
    <div class="carousel-inner carousel-position-avis">
      <div class="carousel-item active">
        <div class="icone-quote">
          <img src="img-avis/Avis/icone-quote.png" alt="icone-quote">
        </div>
        <p class="border-avis margin-auto-avis">
          <img src="img-avis/Avis/icone-voiture.png" alt="icone-voiture">
          <span class="text-dark text-size-avis">Renault - Clio</span><br><br><br>
          <span class="text-dark text-size-avis">« J’ai acheté ma nouvelle voiture chez Sport Design Automobiles
           il y a quelques semaines, rapidité et efficacité dans la recherche,
          tarif compétitif, accueil agréable. Je recommande ! »</span>
          <img src="img-avis/Avis/icone-auteur.png" alt="icone-auteur" class="icone-auteur-spacing d-flex flex-column">
          <span class="auteur-icone-name text-size-avis">Mme HENRY</span>
        </p>
      </div>
      <div class="carousel-item">
        <div class="icone-quote">
          <img src="img-avis/Avis/icone-quote.png" alt="icone-quote">
        </div>
        <p class="border-avis margin-auto-avis">
          <img src="img-avis/Avis/icone-voiture.png" alt="icone-voiture">
          <span class="text-dark text-size-avis">Renault - Clio</span><br><br><br>
          <span class="text-dark text-size-avis">« J’ai acheté ma nouvelle voiture chez Sport Design Automobiles
           il y a quelques semaines, rapidité et efficacité dans la recherche,
          tarif compétitif, accueil agréable. Je recommande ! »</span>
          <img src="img-avis/Avis/icone-auteur.png" alt="icone-auteur" class="icone-auteur-spacing d-flex flex-column">
          <span class="auteur-icone-name text-size-avis">Mme HENRY</span>
        </p>
      </div>
      <div class="carousel-item">
        <div class="icone-quote">
          <img src="img-avis/Avis/icone-quote.png" alt="icone-quote">
        </div>
        <p class="border-avis margin-auto-avis">
          <img src="img-avis/Avis/icone-voiture.png" alt="icone-voiture">
          <span class="text-dark text-size-avis">Renault - Clio</span><br><br><br>
          <span class="text-dark text-size-avis">« J’ai acheté ma nouvelle voiture chez Sport Design Automobiles
           il y a quelques semaines, rapidité et efficacité dans la recherche,
          tarif compétitif, accueil agréable. Je recommande ! »</span>
          <img src="img-avis/Avis/icone-auteur.png" alt="icone-auteur" class="icone-auteur-spacing d-flex flex-column">
          <span class="auteur-icone-name text-size-avis">Mme HENRY</span>
        </p>
      </div>
      <div class="carousel-item">
        <div class="icone-quote">
          <img src="img-avis/Avis/icone-quote.png" alt="icone-quote">
        </div>
        <p class="border-avis margin-auto-avis">
          <img src="img-avis/Avis/icone-voiture.png" alt="icone-voiture">
          <span class="text-dark text-size-avis">Renault - Clio</span><br><br><br>
          <span class="text-dark text-size-avis">« J’ai acheté ma nouvelle voiture chez Sport Design Automobiles
           il y a quelques semaines, rapidité et efficacité dans la recherche,
          tarif compétitif, accueil agréable. Je recommande ! »</span>
          <img src="img-avis/Avis/icone-auteur.png" alt="icone-auteur" class="icone-auteur-spacing d-flex flex-column">
          <span class="auteur-icone-name text-size-avis">Mme HENRY</span>
        </p>
      </div>
      <div class="carousel-item">
        <div class="icone-quote">
          <img src="img-avis/Avis/icone-quote.png" alt="icone-quote">
        </div>
        <p class="border-avis margin-auto-avis">
          <img src="img-avis/Avis/icone-voiture.png" alt="icone-voiture">
          <span class="text-dark text-size-avis">Renault - Clio</span><br><br><br>
          <span class="text-dark text-size-avis">« J’ai acheté ma nouvelle voiture chez Sport Design Automobiles
           il y a quelques semaines, rapidité et efficacité dans la recherche,
          tarif compétitif, accueil agréable. Je recommande ! »</span>
          <img src="img-avis/Avis/icone-auteur.png" alt="icone-auteur" class="icone-auteur-spacing d-flex flex-column">
          <span class="auteur-icone-name text-size-avis">Mme HENRY</span>
        </p>
      </div>
      <div class="carousel-item">
        <div class="icone-quote">
          <img src="img-avis/Avis/icone-quote.png" alt="icone-quote">
        </div>
        <p class="border-avis margin-auto-avis">
          <img src="img-avis/Avis/icone-voiture.png" alt="icone-voiture">
          <span class="text-dark text-size-avis">Renault - Clio</span><br><br><br>
          <span class="text-dark text-size-avis">« J’ai acheté ma nouvelle voiture chez Sport Design Automobiles
           il y a quelques semaines, rapidité et efficacité dans la recherche,
          tarif compétitif, accueil agréable. Je recommande ! »</span>
          <img src="img-avis/Avis/icone-auteur.png" alt="icone-auteur" class="icone-auteur-spacing d-flex flex-column">
          <span class="auteur-icone-name text-size-avis">Mme HENRY</span>
        </p>
      </div>
      <div class="carousel-item">
        <div class="icone-quote">
          <img src="img-avis/Avis/icone-quote.png" alt="icone-quote">
        </div>
        <p class="border-avis margin-auto-avis">
          <img src="img-avis/Avis/icone-voiture.png" alt="icone-voiture">
          <span class="text-dark text-size-avis">Renault - Clio</span><br><br><br>
          <span class="text-dark text-size-avis">« J’ai acheté ma nouvelle voiture chez Sport Design Automobiles
           il y a quelques semaines, rapidité et efficacité dans la recherche,
          tarif compétitif, accueil agréable. Je recommande ! »</span>
          <img src="img-avis/Avis/icone-auteur.png" alt="icone-auteur" class="icone-auteur-spacing d-flex flex-column">
          <span class="auteur-icone-name text-size-avis">Mme HENRY</span>
        </p>
      </div>
    </div>
    <a class="carousel-control-prev" href="#carousel" data-slide="prev">
      <span><img src="img-avis/Avis/nav-left.png" alt="nav-left" class="nav-left"></span>
    </a>
    <a class="carousel-control-next" href="#carousel" data-slide="next">
      <span><img src="img-avis/Avis/nav-right.png" alt="nav-right" class="nav-right"></span>
    </a>
  </div>
  <!--END CAROUSEL-->
</section>
<!-- End Avis-->

<!-- Start Form -->
<section class="bg-image-form">
  <div class="container pt-5">
    <h3 class="text-uppercase color-red-footer size-text-form">Votre avis nous intéresse !</h3>
  </div>
  <div class="container">
    <div class="row pt-4">
        <div class="col-lg-6 col-sm-4 text-uppercase text-white">
            <div class="form-example pb-4">
              <input type="text" id="name" name="name" required
                 minlength="4" maxlength="20" size="40" class="bg-transparent border-form" placeholder="nom">
            </div>
            <div class="form-example pb-4">
              <input type="email" name="email" id="email"  size="40" required class="bg-transparent border-form" placeholder="email">
            </div>
            <div class="form-example pb-4">
              <textarea rows = "15" cols = "40" name = "description" class="bg-transparent border-form" placeholder="message"></textarea>
            </div>
            <div class="form-example form-button pb-4">
              <button type="button" class="btn btn-outline-danger text-uppercase size-text-form mt-4">Envoyer</button>
            </div>
          </div>
          <div class="col-lg-6 col-sm-4 text-uppercase text-white">
            <div class="form-example pb-4">
              <input type="text" id="marque" name="marque" required
                 minlength="4" maxlength="20" size="40" class="bg-transparent border-form" placeholder="marque">
            </div>
            <div class="form-example pb-4">
              <input type="text" id="modele" name="modele" required
                 minlength="4" maxlength="20" size="40" class="bg-transparent border-form" placeholder="modele">
            </div>
            <h6 class="text-uppercase text-white size-text-form pt-4 pl-4">Qualité de notre accueil</h6>
              <ul class="d-flex flex-row justify-content-center margin-stars  pb-5">
                <li>
                  <span><img src="img-avis/Form/star-red.png" alt="star-red" class="mr-4 stars-width-mobile"></span>
                </li>
                <li>
                  <span><img src="img-avis/Form/star-red.png" alt="star-red" class="mr-4 stars-width-mobile"></span>
                </li>
                <li>
                  <span><img src="img-avis/Form/star-red.png" alt="star-red" class="mr-4 stars-width-mobile"></span>
                </li>
                <li>
                  <span><img src="img-avis/Form/star-red.png" alt="star-red" class="mr-4 stars-width-mobile"></span>
                </li>
                <li>
                  <span><img src="img-avis/Form/star-white.png" alt="star-white" class="mr-4 stars-width-mobile"></span>
                </li>
              </ul>
              <h6 class="text-uppercase text-white size-text-form pl-4">Nos infrastructures</h6>
                <ul class="d-flex flex-row justify-content-center margin-stars">
                  <li>
                    <span><img src="img-avis/Form/star-red.png" alt="star-red" class="mr-4 stars-width-mobile"></span>
                  </li>
                  <li>
                    <span><img src="img-avis/Form/star-red.png" alt="star-red" class="mr-4 stars-width-mobile"></span>
                  </li>
                  <li>
                    <span><img src="img-avis/Form/star-red.png" alt="star-red" class="mr-4 stars-width-mobile"></span>
                  </li>
                  <li>
                    <span><img src="img-avis/Form/star-red.png" alt="star-red" class="mr-4 stars-width-mobile"></span>
                  </li>
                  <li>
                    <span><img src="img-avis/Form/star-white.png" alt="star-white" class="mr-4 stars-width-mobile"></span>
                  </li>
                </ul>
                <div class="form-example pb-4 mobile-button">
                  <button type="button" class="btn btn-outline-danger text-uppercase size-text-form mt-4">Envoyer</button>
                </div>
              </div>
              <!-- Fin div row -->
            </div>
          </div>
        </section>
    <!-- End Form -->


  <?php
  include('footer.php');
  ?>
